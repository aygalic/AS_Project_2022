...
final_dataset = reduced_data_treatment
final_dataset[is.na(reduced_data_treatment)]= 0.5
sum(is.na(final_dataset))
```

```{r}
colMeans(final_dataset, na.rm = TRUE)
```


```{r}
x<-colMeans(final_dataset, na.rm = TRUE)-colMeans(reduced_data_treatment, na.rm = TRUE)
hist(x*100, main="Influence of Na removing on the Cell Line AUC means",xlab = "Mean difference (%)")
```

```{r}
y=rowMeans(final_dataset, na.rm = TRUE)-rowMeans(reduced_data_treatment, na.rm = TRUE)
y
hist(y*100, main="Influence of Na removing on the Treatment AUC means", xlab = "Mean difference (%)", probability = FALSE )
```
```{r}
hist(rowMeans(final_dataset))
final_dataset
```
```{r}
sum(rowMeans(final_dataset, na.rm = TRUE)>0.94)
reduced_data_treatment =final_dataset[rowMeans(final_dataset, na.rm = TRUE)>0.94,]
reduced_data_treatment = t(reduced_data_treatment)
```
...
PC123 <- NULL
for(i in 1:43)PC123 <- rbind(PC123, pc$loadings[,1]*pc$scores[i,1] + pc$loadings[,2]*pc$scores[i,2] + pc$loadings[,3]*pc$scores[i,3] + M)
points3d(PC123, col='red', size=6)

for(i in 1:43)lines3d(rbind(reduced_data_treatment[i,], PC123[i,]),col='blue')
...
It may be interesting if we find a way to make groups of cell line or treatment and after repeat this plot.
