...
c(FBL,RPS16, REG1A, PDIA4, AC0040571, CTD2328D61)
...
# abbiamo un'approssimazione a due decimali, per evitare che si sovrappongano aggiungiamo errore
# questo viene fatto anche per aggiungere robustezza all'analisi [chiaramente piccolo errore]
cat("Let us first see if there is normality and a difference in the groups\n")
if(g==2) 
  #two groups are true and false
  {
  true = X[  group==group_names[1], ]
  false = X[  group==group_names[2], ]
  print(mcshapiro.test(true))
  print(mcshapiro.test(false))
  
  ts<- test.manova_g2(true, false)
  print(ts)
  S1<- ts$S1
  S2<-ts$S2
  par(mfrow=c(1,2))
...
image(S2, col=heat.colors(100),main='Cov. S2', asp=1, axes = FALSE, breaks = quantile(rbind(S1,S2), (0:100)/100, na.rm=TRUE))
  ...
  Var <- list()
  for (i in 1:g){
    Var[[i]] <- cov(X[  group==group_names[i], ])
  }
  print(Var)
  
  S  <-  cov(X)
  
  # Qualitatively: [it doesn't look to have the same covariance]
  for(i in 1:g){
    round(Var[[i]],digits=1)
  }
