...
for(i in 1:k){
  df <- rbind(df, data.frame(x = v1, y = v2, frame = rep(i, size), name = names,
                             clust = kmeans(M_scaled, centers=i)$cluster))
}
...
for(algo in algos){
  ...
  for(i in 1:k){
    df <- rbind(df, data.frame(x = v1, y = v2, frame = rep(i ,size), name = names,
                               clust = hcut(M_scaled, i, hc_method = algo[2], hc_metric= algo[1])$cluster))
  }
...
saveWidget(fig, "output/aygalic/Clust_genes.html", selfcontained = F, libdir = "lib")
